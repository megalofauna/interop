...
import type { ActivationRegistration } from "./interop-activation.service";

describe("InteropActivation", () => {
  ...
  describe("trigger behavior and runtime controls", () => {
    ...
    it("prevents reentrant async execution by default", async () => {
      ...
      const calls: string[] = [];
      let firstDoneResolve: (() => void) | undefined;
      const firstDone = new Promise<void>((resolve) => {
        firstDoneResolve = resolve;
      });
      ...
      // Resolve first execution and wait for completion
      resolvers[0]();
      await firstDone;
