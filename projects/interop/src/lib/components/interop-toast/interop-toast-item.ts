...
import {
	...
	INTEROP_TOAST_DEFAULTS,
} from "./interop-toast.config";
	...
	template: `
		<div class="interop-toast-item__content">
			...
			@if (toast().description) {
				<div class="interop-toast-item__description">
					{{ toast().description }}
				</div>
			}
		</div>
...
export class InteropToastItem implements OnDestroy {
	...
	protected readonly state = signal<"entering" | "visible" | "exiting">(
		"entering",
	);
	...
	constructor() {
		...
		effect(() => {
			...
			if (isPaused || toast.duration <= 0 || !isFinite(toast.duration)) {
				...
				if (isPaused && this.timerStartedAt > 0) {
					...
					this.timeRemaining = Math.max(
						0,
						(this.timeRemaining || toast.duration) - elapsed,
					);
				}
				return;
			}
			...
			const duration =
				this.timeRemaining > 0 ? this.timeRemaining : toast.duration;
	...
	private setupSwipe(): void {
		const swipeDismiss =
			this.globalConfig.swipeDismiss ?? INTEROP_TOAST_DEFAULTS.swipeDismiss;
		if (!swipeDismiss) return;

		const el = this.el.nativeElement;
		const threshold =
			this.globalConfig.swipeThreshold ?? INTEROP_TOAST_DEFAULTS.swipeThreshold;
