...
export class InteropToastViewport implements OnDestroy {
	...
	protected readonly effectivePosition = computed<ToastPosition>(
		() =>
			this.position() ??
			this.globalConfig.position ??
			INTEROP_TOAST_DEFAULTS.position,
	);

	private readonly effectiveMaxVisible = computed<number>(
		() =>
			this.maxVisible() ??
			this.globalConfig.maxVisible ??
			INTEROP_TOAST_DEFAULTS.maxVisible,
	);

	private readonly effectiveHotkey = computed<string>(
		() =>
			this.hotkey() ??
			this.globalConfig.hotkey ??
			INTEROP_TOAST_DEFAULTS.hotkey,
	);
	...
	private setupVisibilityListener(): void {
		...
		this.visibilityCleanup = () =>
			this.doc.removeEventListener("visibilitychange", handler);
	}
}
