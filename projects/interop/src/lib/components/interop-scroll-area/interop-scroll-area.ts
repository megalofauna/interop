...
export class InteropScrollArea implements OnDestroy {
	...
	orientation = input<"vertical" | "horizontal" | "both" | undefined>(
		undefined,
	);
	...
	readonly scrollDirection = signal<"up" | "down" | "left" | "right" | "idle">(
		"idle",
	);
	...
	>(
		() =>
			this.orientation() ??
			this.globalConfig.orientation ??
			INTEROP_SCROLL_AREA_DEFAULTS.orientation,
	);
	protected readonly effectiveShowShadows = computed<boolean>(
		() =>
			this.showShadows() ??
			this.globalConfig.showShadows ??
			INTEROP_SCROLL_AREA_DEFAULTS.showShadows,
	);
	private readonly effectiveShadowThreshold = computed<number>(
		() =>
			this.shadowThreshold() ??
			this.globalConfig.shadowThreshold ??
			INTEROP_SCROLL_AREA_DEFAULTS.shadowThreshold,
	);
	...
	scrollToBottom(behavior: ScrollBehavior = "smooth"): Promise<void> {
		return this.scrollTo({
			top: this.hostEl.nativeElement.scrollHeight,
			behavior,
		});
	}
	...
	private checkOverflow(): void {
		...
		const overflowsY =
			orientation !== "horizontal" && el.scrollHeight > el.clientHeight + 1;
		const overflowsX =
			orientation !== "vertical" && el.scrollWidth > el.clientWidth + 1;
	...
	private updateScrollState(): void {
		...
		if (this.effectiveShowShadows()) {
			...
			el.style.setProperty(
				"--_shadow-top",
				String(Math.min(scrollTop / threshold, 1)),
			);
			el.style.setProperty(
				"--_shadow-bottom",
				String(
					Math.min(
						Math.max(scrollHeight - scrollTop - clientHeight, 0) / threshold,
						1,
					),
				),
			);
			el.style.setProperty(
				"--_shadow-start",
				String(Math.min(scrollLeft / threshold, 1)),
			);
			el.style.setProperty(
				"--_shadow-end",
				String(
					Math.min(
						Math.max(scrollWidth - scrollLeft - clientWidth, 0) / threshold,
						1,
					),
				),
			);
