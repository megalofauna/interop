...
import { TablerCircleCheckFilled } from "../../iconsets/tabler/filled/tabler-circle-check-filled";
import {
...
@Component({
					...
					@switch (effectiveStatus()) {
						@case ("completed") {
							<interop-icon
								...
								"
								[size]="28"
								[color]="'var(--itx-neutral-14)'"
							/>
						}
						@case ("error") {
							<interop-icon
								[name]="resolvedIcons().error ?? 'tabler-alert-circle'"
								[size]="16"
							/>
						}
						@case ("skipped") {
							<interop-icon
								[name]="resolvedIcons().skipped ?? 'tabler-minus'"
								[size]="16"
							/>
						}
						@default {
	...
	providers: [provideInteropIcons(TablerCircleCheckFilled)],
})
export class InteropStep implements OnDestroy {
	...
	constructor() {
		...
		if (isDevMode()) {
			if (!this.stepper) {
				afterNextRender(() => {
					console.warn("interop-step: must be used inside <interop-stepper>.");
				});
			}
		}
	}
	...
	protected readonly isActive = computed(
		() =>
			!this.stepper?.isFinished() && this.stepper?.activeIndex() === this.index,
	);
	 ...
	 */
	protected readonly resolvedIcons = computed(
		(): Partial<Record<StepStatus, string>> => ({
			...(this.stepper?.icons() ?? {}),
			...this.icons(),
		}),
	);
