 ...
 */
/*
 * SPIKE — identical assertions to elevation.spec.ts, with ONLY the engine
 * swapped for the computed one. If this passes, the computed engine is
 * behaviourally indistinguishable from the unrolled one it replaces.
 */
import { LADDER_CSS } from "./ladder.css-source";
import { SPIKE_ENGINE_CSS } from "./elevation-spike.css-source";

describe("Layer engine (SPIKE — computed)", () => {
