...
export class InteropAttrs {
  ...
  withOptOut(
    ...
    for (const [selector, attrs] of Object.entries(config)) {
      ...
      if (shouldConstrain) {
        // Constrain only the broad direct-child wildcard pattern.
        // NOTE: We intentionally operate on normalized text here.
        const constrained = normalized.replace(
          /^:host\s*>\s*\*$/g,
