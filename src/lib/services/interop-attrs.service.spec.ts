...
import {
  ...
  SetAttrsConfig,
  PresetKey,
} from "./interop-attrs.service";

describe("InteropAttrs", () => {
  let service: InteropAttrs;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(InteropAttrs);
  ...
  describe("deriveObserverOptions", () => {
    it("should return shallow observation for host-only selectors", () => {
      ...
      expect(result.subtree).toBe(true); // Default to subtree when selectors aren't all shallow
    });
  ...
  describe("applyConfig", () => {
    ...
    it("should apply attributes to child elements", () => {
      ...
      spyOn(hostElement, "querySelectorAll").and.returnValue(
        Array.from(hostElement.querySelectorAll(".child")) as any,
      );
