 ...
 * the --ntr-layout-* CSS variables in their stylesheets.
 *
 * Key features:
...
export class InteropLayoutDirective {
  ...
  private validateLayoutConfig(config: LayoutConfig): void {
    ...
    if (!isLayoutCapable) {
      console.warn(
        ...
        `Consider using on container elements or components that consume --ntr-layout-* variables.`,
        element
      );
    }

    // Warn about potentially conflicting properties
    if (config.direction?.includes('column') && config.justify === 'between' && !config.gap) {
      console.warn(
  ...
  private checkLayoutCapability(element: HTMLElement): boolean {
    ...
    const containerElements = ['div', 'section', 'header', 'footer', 'nav', 'main'];
