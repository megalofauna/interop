 ...
 * Components implementing this interface should consume --ntr-layout-* CSS variables.
 */
export interface LayoutCapable {
  readonly supportsLayout: true;
  readonly supportedLayoutProps: ReadonlyArray<'direction' | 'justify' | 'align' | 'wrap' | 'gap'>;
}
 ...
 */
export function LayoutCapable(supportedProps: Array<'direction' | 'justify' | 'align' | 'wrap' | 'gap'> = ['direction', 'justify', 'align', 'wrap', 'gap']) {
...
export function checkElementLayoutCapability(element: HTMLElement): {
  ...
  if (interopLayoutComponents.includes(tagName)) {
    return { isCapable: true, reason: 'Known layout-capable Interop component' };
  }

  // Generic container elements
  const containerElements = ['div', 'section', 'header', 'footer', 'nav', 'main', 'article', 'aside'];
  ...
  if (computedStyle.display === 'grid') {
    return { isCapable: true, reason: 'Element has display: grid (partial support)' };
  }

  // Check for layout-related CSS variables
  const hasLayoutVars = [
    '--ntr-layout-direction',
    '--ntr-layout-justify',
    '--ntr-layout-align',
    '--ntr-layout-wrap',
    '--ntr-layout-gap'
  ...
  if (nonLayoutElements.includes(tagName)) {
    return { isCapable: false, reason: `${tagName} elements are typically not layout containers` };
  }
 ...
 */
export function getLayoutVariableInfo(element: HTMLElement): Record<string, string | null> {
  const computedStyle = getComputedStyle(element);

  return {
    direction: computedStyle.getPropertyValue('--ntr-layout-direction') || null,
    justify: computedStyle.getPropertyValue('--ntr-layout-justify') || null,
    align: computedStyle.getPropertyValue('--ntr-layout-align') || null,
    wrap: computedStyle.getPropertyValue('--ntr-layout-wrap') || null,
    gap: computedStyle.getPropertyValue('--ntr-layout-gap') || null
  };
}
