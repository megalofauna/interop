...
/**
 * InteropRadio - Enhanced radio input component with activation guardrails and coordination.
 *
...
@Component({
  ...
  `,
  styleUrl: "./interop-radio.css",
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class InteropRadio {
   ...
   * When set, this radio triggers a handler registered with ActivationManagerService.
  ...
  constructor() {
    ...
    if (isDevMode()) {
      afterNextRender(() => {
        ...
        if (element.tagName !== "LABEL") {
          console.warn(
            "InteropRadio must be used on <label> elements for semantic correctness. " +
    ...
    effect(() => {
      ...
      if (id && !this.onActivate()) {
        ...
        console.warn(
          `InteropRadio: an activationId has been specified ("${id}"), but no corresponding activation handler has been registered with Interop. ` +
