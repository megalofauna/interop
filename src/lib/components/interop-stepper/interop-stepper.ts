...
@Component({
  ...
  standalone: true,
  imports: [InteropButton, InteropIcon, InteropListbox],
...
export class InteropStepper implements IInteropStepper, AfterViewInit, OnDestroy {
  ...
  /** Stable id for the popover element so the trigger can target it. */
  protected readonly menuId = `interop-stepper-menu-${nextStepperId++}`;

  /** Per-instance anchor name. CSS anchor positioning binds to the LAST
   * element in source order when multiple share an anchor-name, so each
   * stepper needs its own unique name to avoid every popover stacking
   * against the same trigger. */
  protected readonly menuAnchorName = `--${this.menuId}-anchor`;

  private readonly menuPopover = viewChild<ElementRef<HTMLElement>>("menuPopover");
  ...
  /**
   * Auto-status is derived purely from `activeIndex` and the completion
   * `_frontier`. Exactly one step can be "active" at a time (the current
   * one), and "completed" is everything strictly before the frontier.
